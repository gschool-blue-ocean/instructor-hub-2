...
//===============================================================================
//-----------------Student Routes--(GET, POST, PATCH, DELETE)--------------------
//===============================================================================
...
app.get("/api/students/:cohort_name", authenticateToken, (req, res) => {
...
app.post("/api/students/add", async (req, res) => {
  let newStudentData = req.body.newStudentData;
  const promises = newStudentData.map((student) => {
    const studentName = student.name;
    const studentEmail = student.email;
    const studentGithub = student.github;
    const studentCohort = student.cohortName
    return pool.query(
      `INSERT INTO students (name, email, github, cohort_name) VALUES ($1, $2, $3) RETURNING *`,
      [studentName, studentEmail, studentGithub, studentCohort]
    );
  });
...
//send a PATCH request to edit selected student info.
app.patch(`/api/students/:id`, authenticateToken, async (req, res) => {
  const { name, email, github, cohort_name } = req.body;
  const { id } = req.params;
  ...
  console.log(updateQuery)
  if (cohort_name) {
    updateQuery += `cohort_name = $${index}, `;
    console.log(values)
    values.push(cohort_name);
    console.log(values)
    index++;
  }
  console.log(updateQuery)
  
  updateQuery = updateQuery.slice(0, -2) + ` WHERE student_id = $${index}`;
  console.log(values)
  values.push(id);
  console.log(values)
...
app.get("/images/:email", (req, res) => {
  ...
  pool
    .query(`SELECT img FROM users WHERE email = $1 `, [email])
    .then((result) => res.send(result.rows))
    .catch((error) => res.send(error));
});
app.patch("/update/img", (req, res) => {
  ...
  pool
    .query("UPDATE users SET img = $1 WHERE email = $2 RETURNING img", [
      img,
      user,
    ])
    .then((result) => res.status(200).send(result.rows))
...
app.post("/api/create/user", (req, res) => {
  ...
  const accountToken = jwt.sign({ id: user.user_id }, random);
  ...
  bcrypt.hash(user.password, saltRounds, (err, hash) => {
    ...
    pool
      .query(
        `INSERT INTO users (email, password, img, token, session_token) VALUES ($1, $2,'/image/f84dd5fb0bda1ba2d636c0bfdc795115', $3, $4) ON CONFLICT (email) DO NOTHING RETURNING *`,
        [user.email, hash, accountToken, sessionToken]
      )
